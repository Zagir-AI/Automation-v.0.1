...
13. ✅ Дашборд на стартовом экране
    - ui/app.py: блок `if not active_path:` заменён на полноценный дашборд
    - 4 метрики: всего проектов / рассчитано / суммарная Pуст / нарушения кабелей
    - Карточки 2-в-ряд: Pуст/Pрасч, Iвру/cosφ, кат.ПУЭ, дата расчёта, кнопка "→ Открыть"
    - Регрессия DEMO: ✅

14. ✅ Расчёт объёмов земляных работ для кабельных траншей
    - outdoor/calc_earthwork.py: calc_trench(), calc_all_earthwork()
    - Нормативы: СНиП 3.05.06-85, ПУЭ 2.3.83, ГЭСН 81-02-01
    - Ширина: 1к→0.30м, 2к→0.50м, 3+к→0.50+(n-2)×0.15м
    - Объёмы: выемка, вывоз с разрыхлением, песок (0.20м), обратная засыпка
    - Защита: кирпич 250×120мм или плита 1000×800мм
    - Опциональный блок "trench" в outdoor_networks для override параметров
...
## Дисциплины проекта (разделы)

| Раздел | Полное название | Аналог в системе | Примечание |
|--------|----------------|------------------|------------|
| **ЭОМ** | Электрооборудование и механизмы | `feeder.section="ЭОМ"` | Внутренние силовые и осветительные сети, щиты |
| **ЭО** | Электрическое освещение | `feeder.section="Освещение"` / аналог | Рабочее, аварийное, эвакуационное освещение |
| **НО** | Наружное освещение | `outdoor_networks[]` | Паркинг, территория, дворовое освещение |
| **ЭН** | Наружные сети электроосвещения | `outdoor_networks[]` + `feeder.section="ЭН"` | Аналогично НО; улицы, дороги, ШУНО |
| **ЭС** | Наружные сети электроснабжения | отдельная модель (ТП, кабели ВН) | Ввод от ТП, кабели 10 кВ, трансформатор |

> **Важно**: ЭН ≠ электроснабжение. ЭН = наружные сети электроОСВЕЩЕНИЯ (как НО).
> ЭС = наружные сети электроСНАБЖЕНИЯ (ТП, трансформатор, ввод).
> В auto_panels: `"ЭН": "ШУНО"` — корректно (ШУНО = щит управления наружным освещением).

## В очереди
Все задачи из ROADMAP.md завершены (задачи #1–15). Проект в актуальном состоянии.

Новые задачи (текущий спринт UI):
- ✅ ЭОМ: генератор розеточной сети (Socket auto-grouping)
- ✅ ЭОМ/ЭО: редактор помещений + генератор осветительных групп (rooms[] + СП 52)
- ✅ НО/ЭН: вкладка «🌃 НО» — редактор outdoor_networks[], результаты расчёта
- 🔄 ЭС: вкладка наружных сетей электроснабжения (ТП, параметры трансформатора)
