...
def cmd_sld(args):
    """Генерация детальных однолинейных схем щитов (DXF, ГОСТ 21.608-84)."""
    proj_dir = find_project_dir(args.path)
    project  = load_project(proj_dir)
    project  = _ensure_calc(project, proj_dir)
    ...
    p_name  = project.get("project", {}).get("name", "")
    out_dir = proj_dir / "dwg"
    out_dir.mkdir(exist_ok=True)

    print(hdr(f"Однолинейные схемы щитов: {p_name}"))

    if getattr(args, "panel", None):
        path = generate_panel_sld(project, args.panel, out_dir)
        if path:
            print(ok(f"Схема создана: {path.name}"))
            print(info(f"Файл: {path}"))
        else:
            print(warn("ezdxf не установлен или щит не найден — "
                       "установи: pip install ezdxf"))
    else:
        paths = generate_all_sld(project, out_dir)
        if not paths:
            print(warn("ezdxf не установлен или нет щитов — "
                       "установи: pip install ezdxf"))
            return
        for p in paths:
            print(ok(f"  {p.name}"))
        print()
        print(ok(f"Всего схем: {len(paths)}"))
        print(info(f"Папка: {out_dir}"))
...
def main():
    ...
    p_sld.add_argument("--panel", default=None,
                        help="ID щита (по умолчанию — все щиты)")
