...
with tab_docs:
    ...
    with col_sld1:
        if st.button("📐 Все щиты (DXF)", key="sld_all", width="stretch"):
            with st.spinner("Генерация однолинейных схем..."):
    ...
    with col_sld2:
        dwg_dir_sld = proj_dir / "dwg"
        dxf_files   = sorted(dwg_dir_sld.glob("*_sld.dxf")) if dwg_dir_sld.exists() else []
        if dxf_files:
            selected = st.selectbox(
                "Скачать файл",
                [f.name for f in dxf_files],
                key="sld_dl_sel",
            )
            chosen = next((f for f in dxf_files if f.name == selected), None)
            if chosen:
                st.download_button(
                    "⬇ Скачать DXF",
            ...
            st.caption("DXF-файлы не найдены. Нажми «Все щиты (DXF)» для генерации.")
